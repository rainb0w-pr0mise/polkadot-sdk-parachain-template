...
fn set_dummy_works() {
    new_test_ext().execute_with(|| {
        // Ensure that setting a dummy value works
        assert_ok!(BalanceGame::set_dummy(RuntimeOrigin::root(), 1, 100));
        assert_eq!(Dummy::<Test>::get(1), Some(100));

        // Ensure that the event is emitted
        System::assert_last_event(Event::SetDummy { balance: 100 }.into());
    });
}
...
fn set_dummy_fails_if_value_already_set() {
    new_test_ext().execute_with(|| {
        // Set a dummy value for account 1
        assert_ok!(BalanceGame::set_dummy(RuntimeOrigin::root(), 1, 100));

        // Ensure that setting a dummy value again fails
        assert_noop!(
            BalanceGame::set_dummy(RuntimeOrigin::root(), 1, 200),
...
fn accumulate_dummy_works() {
    new_test_ext().execute_with(|| {
        ...
        // Accumulate the dummy value
        assert_ok!(BalanceGame::accumulate_dummy(RuntimeOrigin::signed(1), 50));
        assert_eq!(Dummy::<Test>::get(1), Some(150));

        System::assert_last_event(Event::AccumulateDummy { balance: 50 }.into());
    });
}

#[test]
fn accumulate_dummy_fails_if_no_entry_for_sender() {
    new_test_ext().execute_with(|| {
        // Ensure that accumulating a dummy value fails if no entry exists for the sender
        assert_noop!(
            BalanceGame::accumulate_dummy(RuntimeOrigin::signed(1), 50),
...
fn update_balance_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Bar::<Test>::get(1), Some(100));
        assert_eq!(Foo::<Test>::get(), 100);
...
fn check_win_condition_in_on_finalize_works() {
    new_test_ext().execute_with(|| {
        // Set a dummy value for account 1
        assert_ok!(BalanceGame::set_dummy(
            RuntimeOrigin::root(),
            1,
            1_000_000_000
        ));

        // Update the balance to trigger the win condition
        assert_ok!(BalanceGame::update_balance(RuntimeOrigin::signed(1)));
...
fn operation_limit_exceeded() {
    new_test_ext().execute_with(|| {
        ...
        for _ in 0..31 {
            assert_ok!(BalanceGame::accumulate_dummy(RuntimeOrigin::signed(1), 10));
        }

        // Ensure that the next operation fails due to the operation limit
        assert_noop!(
            BalanceGame::accumulate_dummy(RuntimeOrigin::signed(1), 10),
